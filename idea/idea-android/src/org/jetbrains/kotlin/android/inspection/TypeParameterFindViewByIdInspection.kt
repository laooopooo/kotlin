...
class TypeParameterFindViewByIdInspection : AbstractKotlinInspection(), CleanupLocalInspectionTool {
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean, session: LocalInspectionToolSession): PsiElementVisitor {
        ...
        return object : KtVisitorVoid() {
            override fun visitCallExpression(expression: KtCallExpression) {
                ...
                val parentCast = expression.parent as? KtBinaryExpressionWithTypeRHS ?: return
    ...
    class ConvertCastToFindViewByIdWithTypeParameter : LocalQuickFix {
        ...
        private fun KtCallExpression.addTypeArgument(type: KtTypeProjection) {
            if (typeArgumentList != null) {
                typeArgumentList?.addArgument(type)
            }
            else {
                addAfter(KtPsiFactory(this).createTypeArguments("<${type.text}>"), calleeExpression)
            }
        }
    }
